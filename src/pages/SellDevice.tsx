  ...
  { name: string; 
    label: string;
     required?: boolean;
    type?: "text" | "number" | "textarea" | "yesno"
   };
...
if(ramType){
   if (!/^DDR[3-5]$/i.test(ramType.toString().trim())) {
...
if(ramSpeed){
 if (!/^\d+$/.test(ramSpeed.toString().trim())) {
...
case "Hard Disk": {
 ...
 if(type){
  if (!/^(HDD|SSD)$/i.test(type.toString().trim())) {
...
case "Monitor": {
  ...
  if (resolution) {
 if (!/^\d{3,5}\s*([xX*])\s*\d{3,5}$/.test(resolution)) {
